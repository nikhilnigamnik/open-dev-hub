export const blogRedirect = [
  {
    id: 1,
    title: "Beginner's Guide to React Query",
    link: "beginners-guide-to-react-query",
    author: "Nikhil Nigam",
    content:
      "React Query is a library that helps you fetch and manage data in your React applications. It provides a set of hooks for fetching, caching, and updating data in your application. In this article, we will learn how to use React Query to fetch data from an API and display it in a React component.",
    publishDate: "April 24, 2023",
    tags: ["React", "React Query"],
  },
  {
    id: 2,
    title: "Recoil - A New Approach to Manage State",
    link: "recoil-a-new-approach-to-manage-state",
    author: "Nikhil Nigam",
    content:
      "Recoil is a new state management library for React that lets you manage global state with ease. Recoil is a new approach to manage state.",
    publishDate: "April 24, 2023",
    tags: ["React", "Recoil"],
  },
  {
    id: 3,
    title: "Zustand - A Simple and Fast State Management",
    link: "zustand-state-management-for-react",
    author: "Nikhil Nigam",
    content:
      "Zustand is a simple and fast state management library for React. It provides a way to manage global state in your React applications without the need for a complex setup. In this article, we will learn how to use Zustand to manage state in a React application.",
    publishDate: "April 25, 2023",
    tags: ["React", "Zustand"],
  },
];
